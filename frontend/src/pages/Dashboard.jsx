        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          {progress>0 && (
            <button onClick={()=>onTest(course._id)} disabled={testLoading===course._id}
              style={{ padding:'9px 12px', borderRadius:9, background:'rgba(167,139,250,0.1)', border:'1px solid rgba(167,139,250,0.25)', color:'#a78bfa', fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:11, cursor:testLoading===course._id?'wait':'pointer', opacity:testLoading===course._id?0.6:1, display:'flex', alignItems:'center', gap:5 }}>
              <Zap size={11}/> {testLoading===course._id?'Generating...':'AI Test'}
            </button>
          )}
        </div>
      </div>
...
function CertCard({ course }) {
  ...
  return (
    <motion.div initial={{ opacity:0, scale:.95 }} animate={{ opacity:1, scale:1 }}
      ...
      <button style={{ padding:'7px 12px', borderRadius:8, background:'linear-gradient(135deg,#6366f1,#8b5cf6)', border:'none', color:'#fff', fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:11, cursor:'pointer' }}>
        Download
      </button>
    </motion.div>
  );
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    // Auth abhi load ho rahi hai — wait karo
    ...
    // ✅ ADMIN FIX: admin ko seedha admin panel pe bhejo
    if (user.role === 'admin') {
      navigate('/admin', { replace: true });
      return;
    }

    // Student — data load karo
    setDataLoading(true);
  ...
  }, [user, authLoading]); // authLoading bhi dependency mein
  ...
  // Auth load ho rahi hai ya user check pending hai
  ...
  if (dataLoading) {
    return (
      <div style={{ minHeight:'100vh', background:'#04031a', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
        <div style={{ width:44, height:44, borderRadius:'50%', border:'3px solid rgba(99,102,241,0.2)', borderTopColor:'#6366f1', animation:'spin .8s linear infinite' }}/>
        <p style={{ fontFamily:"'Syne',sans-serif", color:'rgba(255,255,255,0.3)', fontSize:13 }}>Loading your dashboard...</p>
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    );
  }
    ...
    <div style={{ minHeight:'100vh', background:'#04031a', fontFamily:"'DM Sans',sans-serif", overflowX:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:1200, margin:'0 auto', padding:'32px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:16, marginBottom:24 }}>
          <div style={{ background:'linear-gradient(135deg,rgba(99,102,241,0.12),rgba(139,92,246,0.08))', border:'1px solid rgba(99,102,241,0.2)', borderRadius:20, padding:'20px 28px', display:'flex', alignItems:'center', gap:20, minWidth:240 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
            {[
              ...
              <div key={i} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:16, textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:4, marginBottom:24, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:4, width:'fit-content' }}>
          {[{id:'courses',label:'My Courses',icon:BookOpen},{id:'certs',label:'Certificates',icon:Award},{id:'progress',label:'Progress',icon:BarChart2}].map(({id,label,icon:Icon})=>(
            ...
            <motion.div key="courses" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}>
              {enrollments.length===0 ? (
                <div style={{ textAlign:'center', padding:'80px 20px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:20 }}>
                  ...
                  <button onClick={()=>navigate('/courses')} style={{ padding:'12px 28px', borderRadius:10, background:'linear-gradient(135deg,#6366f1,#8b5cf6)', border:'none', color:'#fff', fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:14, cursor:'pointer' }}>Browse Courses →</button>
                </div>
              ) : (
                <>
                  {inProgress.length>0 && (
                    <div style={{ marginBottom:28 }}>
                      <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:'rgba(255,255,255,0.4)', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:14 }}>▸ In Progress</h2>
                  ...
                  {completed.length>0 && (
                    <div style={{ marginBottom:28 }}>
                      <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:'rgba(255,255,255,0.4)', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:14 }}>▸ Completed</h2>
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:16 }}>
                  ...
                  {notStarted.length>0 && (
                    <div>
                      <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:'rgba(255,255,255,0.4)', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:14 }}>▸ Not Started</h2>
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:16 }}>
            ...
            <motion.div key="progress" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} style={{ display:'flex', flexDirection:'column', gap:20 }}>
              ...
              <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:18, padding:24 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  {enrollments.length===0 && <p style={{ color:'rgba(255,255,255,0.3)', textAlign:'center', padding:24 }}>No enrollments yet</p>}
                  {enrollments.map((e,i)=>{
